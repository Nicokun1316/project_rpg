...
namespace UI {
    public class ShopItemComponent : GenericItemComponent<Item> {
        private Image image;

        private TMP_Text text;
        private TMP_Text money;
        private ShopUIData shopData;
        private void Start() {
            image = transform.Find("Image").GetComponent<Image>();
            text = transform.Find("Text").GetComponent<TMP_Text>();
            money = transform.Find("Money").GetComponent<TMP_Text>();
            shopData = GetComponentInParent<ShopUIData>();
            InvalidateItem();
        }
        ...
        protected override void InvalidateItem() {
            if (Item != null && Item.icon != null && image != null) {
                ...
                money.text = $"${Item.cost * shopData.Rate}";
            }
        }
    }
}
